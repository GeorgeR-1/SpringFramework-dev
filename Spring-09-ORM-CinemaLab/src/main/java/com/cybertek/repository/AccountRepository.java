...
// Write a derived query to list all accounts with a specific
// country or state
    List<Account> findAccountsByCountryOrState(String country, String state);
//Write a derived query to list all accounts with age lower than
// or equal to a specific value
    List<Account> findAccountsByAgeLessThanEqual(Integer value);
...
// the beginning of the address contains the keyword
    List<Account> findAllByAddressContains(String pattern);
...
//Write a JPQL query to list all admin accounts
    ...
    List<Account> findAccountsByAdminRole();
//Write a JPQL query to sort all accounts with age
    @Query("SELECT a FROM Account a ORDER BY a.age DESC ")
    List<Account> findAllOrderByAgeDesc();
...
//Write a native query to read all accounts that a specific value can be
//containable in the name, address, country, state city
    @Query(value = "SELECT * FROM account_details WHERE name ILIKE concat('%',?1,'%') OR " +
            "country ILIKE concat('%',?1,'%') OR address ILIKE concat('%',?1,'%') OR" +
            "state ILIKE concat('%',?1,'%')",nativeQuery = true)
    List<Account> findByPattern(String pattern);
//Write a native query to read all accounts with an age lower than a specific value
    @Query(value = "SELECT * FROM account_details WHERE age < :age",
    nativeQuery = true)
