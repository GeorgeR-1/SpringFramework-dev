...
//Write a derived query to read a movie with a name
    Optional<Movie> findAllByName(String movieName);
//Write a derived query to list all movies between a range of prices
    List<Movie> findAllByPriceBetween(BigDecimal price1,BigDecimal price2);
//Write a derived query to list all movies where duration exists
// in the specific list of duration
    List<Movie> findAllByDurationIn(List<Integer> durations);
//Write a derived query to list all movies with higher than
...
//Write a JPQL query to list all movies between a range of prices
    ...
    List<Movie> findMovieBetweenPrices(BigDecimal priceLow, BigDecimal priceHigh);
//Write a JPQL query that returns all movie names
    @Query("SELECT m.name FROM Movie m")
    List<String> returnAllMovieNames();

// ------------------- Native QUERIES ------------------- //
//Write a native query that returns a movie by name
    @Query(value = "SELECT * FROM movie WHERE name = :name",nativeQuery = true)
    Optional<Movie> returnMovieByName(String name);
//Write a native query that return the list of movies in a specific range of prices
    @Query(value = "SELECT * FROM movie WHERE price BETWEEN ?1 AND ?2",nativeQuery = true)
    List<Movie> returnMoviesByPriceRange(BigDecimal price1, BigDecimal price2);
